...
- **基于 pytest marker 的一键回归** — 通过 `run.py --suite` 选择 smoke / single / business / exception / all，统一处理报告生成
...
```
column-erp-testing/
├── base/                     # 核心框架层：请求调度、参数替换、断言、自动重登
├── common/                   # 公共组件层：断言引擎、数据库连接、动态数据生成、通知
├── conf/                     # 配置层：config.ini、环境信息、全局常量
├── testcase/ERP/
│   ├── Single_Interface/     # 单接口测试：商品/仓库/采购/销售 CRUD
│   ├── Business_Scenario/    # 业务链路测试：采购全链路、销售全链路
│   └── Exception/            # 异常与边界场景测试
...
- 推荐优先使用 `python run.py --suite <suite>`，它会自动处理 Allure 报告生成、环境信息注入和 CI 适配

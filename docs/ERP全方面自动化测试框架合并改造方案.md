...
#### P3 后续可补充的真实业务校验（短期增强）

当前 P3 第一版已经覆盖库存变化、单据状态、单据头基础金额和明细基础金额校验。以下内容不属于当前 P3 第一版的阻塞门槛，也不放入后文“后期改进优化方向”的宏观工程化扩展中。它们属于真实 ERP 业务链路的短期增强项：等 API、UI、数据库闭环、性能测试和 CI 主线全部完成后，如果仍有时间，可以在现有采购/销售 E2E 基础上继续补充。

1. **复杂单据金额口径校验**
   - 在当前数量、单价、明细金额和单据头金额校验基础上，继续补充含税金额、折扣、优惠率、付款优惠、其它费用、欠款等字段之间的关系断言。
   - 当系统对采购金额使用负数、销售金额使用正数或存在含税字段时，以真实数据库字段和页面展示为准，先记录字段含义，再补充更细的断言。
   - 多商品、多明细行场景下，补充明细金额合计、含税合计与单据头汇总字段的一致性校验。
   - 该增强仍只验证单据层金额，不要求验证结算账户余额、应收应付或收付款流水。

2. **账户流水金额校验**
   - 在明确允许本阶段或后续阶段执行付款/收款动作后，再扩展采购付款、销售收款、应付/应收和账户余额变化校验。
   - 需要先由人工确认付款/收款字段、账户余额字段、应收应付字段、收付款单生成规则、反审核/清理规则和允许自动化操作的数据范围。
   - 该增强应独立于库存闭环提交，避免把第一版 E2E 扩大成完整财务专项测试。
...
1. GitHub 托管 Runner 的 PR 和主分支默认门禁只通过 HTTP 访问云端 jshERP，执行 API/UI 轻量收集和连通性检查，不为数据库断言直接开放公网 MySQL。
2. API/UI/E2E 数据库完整闭环和只读性能基线先在本地真实环境手动验收；如后续配置自托管 Runner，只允许主分支或经过 Environment 审批的手动任务使用。
...
| 受控自托管 Runner | 当前默认 CI 已覆盖轻量质量门禁，完整业务闭环和只读性能基线可先在本地真实环境手动验收 | 需要把完整 API/UI/E2E、数据库断言和只读性能基线纳入受控 CI，且 Runner 能稳定访问测试 ERP、MySQL 和基础数据时 |
